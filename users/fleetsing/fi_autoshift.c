...
uint16_t get_alt_repeat_key_keycode_user(uint16_t keycode, uint8_t mods) {
    switch (keycode) {
        case KC_TAB:
            return S(KC_TAB); // Tab reverses to Shift + Tab.
        case S(KC_TAB):
            return KC_TAB; // Shift + Tab reverses to Tab.

        case G(FI_Y):
            return G(FI_Z); // GUI + Y reverses to GUI + Z.
        case G(FI_Z):
            return G(FI_Y); // GUI + Z reverses to GUI + Y.
        case G(FI_C):
            return G(FI_V); // GUI + C reverses to GUI + V.

        case C(FI_Y):
            return C(FI_Z); // Ctrl + Y reverses to Ctrl + Z.
        case C(FI_Z):
            return C(FI_Y); // Ctrl + Z reverses to Ctrl + Y.
        case C(FI_C):
            return C(FI_V); // Ctrl + C reverses to Ctrl + V.

        case FI_LPRN:
            return FI_RPRN; // Left Parenthesis reverses to Right Parenthesis.
        case A(FI_8):
            return A(FI_9); // Left Brakcet reverses to Right Bracket.
        case A(FI_LPRN):
            return A(FI_RPRN); // Left Brace reverses to Right Brace.

        case FI_SECT:
            return FI_HALF; // Less Than reverses to Greater Than.
        case FI_HALF:
            return FI_SECT; // Greater Than reverses to Less Than.
    }
    return KC_TRNS;
}